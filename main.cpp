...
#include <DFRobotDFPlayerMini.h>
 ...
 PlantStates lastWaterState = PlantStates::SATISFIED;
...
DFRobotDFPlayerMini dfPlayer;
 ...
 // function prototypes
 void playPlantStateAudio(PlantStates state);
...
dfPlayer.play(1);   // This right here is just a testing line. Just to make sure the DF Mini player works
...
void updatePlantState(PlantStateContainer *plantStatePtr, BehaviorModes currentTankBehavior, BehaviorModes lastTankBehavior) {
  ...
  constexpr int printFrequency = 10;
  ...
  if (moistureSampleTimer >= MOISTURE_SAMPLING_PERIOD) {
    ...
    } else if (plantStatePtr->moistureLevel <= THIRSTY_UPPER_THRESHOLD &&
               plantStatePtr->moistureLevel > PARCHED_UPPER_THRESHOLD) {

      plantStatePtr->waterSatisfaction = PlantStates::THIRSTY;

    } else if (plantStatePtr->moistureLevel > THIRSTY_UPPER_THRESHOLD &&
               plantStatePtr->moistureLevel <= SATISFIED_UPPER_THRESHOLD) {
  ...
  if (millis() - lastAudioTime > 15000) {   
  playPlantStateAudio(plantStatePtr->waterSatisfaction);
